...
import ActivityCreate from "../src/components/ActivityCreate";
...
describe("<ActivityCreate />", () => {
  ...
  describe("Estructura", () => {
    ...
    it('DeberÃ­a renderizar un label con el texto "Name: "', () => {
      expect(ActivityCreate.find("label").at(0).text()).toEqual("Name: ");
    });

    it('DeberÃ­a renderizar un input con la propiedad "name" igual a "name"', () => {
      expect(ActivityCreate.find('input[name="name"]')).toHaveLength(1);
    });

    it('DeberÃ­a renderizar un label con el texto "Difficulty: "', () => {
      expect(ActivityCreate.find("label").at(1).text()).toEqual("Difficulty: ");
    });

    it('DeberÃ­a renderizar un select con la propiedad "name" igual a "difficulty"', () => {
      expect(ActivityCreate.find('select[name="difficulty"]')).toHaveLength(1);
    });

    it('DeberÃ­a renderizar un label con el texto "Duration: "', () => {
      expect(ActivityCreate.find("label").at(2).text()).toEqual("Duration: ");
    });

    it('DeberÃ­a renderizar un input con la propiedad "name" igual a "duration"', () => {
      expect(ActivityCreate.find('input[name="duration"]')).toHaveLength(1);
    });//---------------------------------

    it('DeberÃ­a renderizar un label con el texto "Season: "', () => {
        expect(ActivityCreate.find("label").at(3).text()).toEqual("Season: ");
    });

    it('DeberÃ­a renderizar un select con la propiedad "name" igual a "season"', () => {
        expect(ActivityCreate.find('select[name="season"]')).toHaveLength(1);
    });

    it('DeberÃ­a renderizar un label con el texto "Country: "', () => {
        expect(ActivityCreate.find("label").at(4).text()).toEqual("Country: ");
    });

    it('DeberÃ­a renderizar un select con la propiedad "name" igual a "countries"', () => {
        expect(ActivityCreate.find('select[name="countries"]')).toHaveLength(1);
    });

    it('DeberÃ­a renderizar un button con "type" igual a "submit" y con texto "To Create"', () => {
